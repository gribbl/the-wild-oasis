...
router
  .group(() => {
    ...
    router
      .group(() => {
        router.get('/', [AccountController, 'edit']).as('edit')
        router.post('/name', [AccountController, 'updateName']).as('update.name')
        router.post('/password', [AccountController, 'updatePassword']).as('update.password')
      })
      .prefix('account')
      .as('account')
